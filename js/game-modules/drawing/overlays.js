...
export function drawTipsOverlay() {
    ...
    ctx.fillText('🎮 CONTROLS: Space/Tap to jump, K for Special Move, see ℹ️ for all controls.', textX, lineY);
    lineY += LINE_SPACING;
    ctx.fillText('⚡ ENERGY: Skills consume energy. Collect 🔥 accelerators to recharge!', textX, lineY);
    lineY += LINE_SPACING;
    ctx.fillText('🛡️ INVINCIBILITY: Hover (H) and Moonwalk (M) grant temporary invincibility.', textX, lineY);
    lineY += LINE_SPACING;
    ctx.fillText('💰 ARMORY: Equip skills to earn cash multipliers (up to 2.5x for Legendary!)', textX, lineY);
    lineY += LINE_SPACING;
    ctx.fillText('🏆 MILESTONES: Large hurdles mark progress. Clear them to earn cash!', textX, lineY);
    lineY += LINE_SPACING;
    ctx.fillText('🐌 OBSTACLES: Avoid them! Collisions cause slowdowns and hurt your score.', textX, lineY);
    lineY += LINE_SPACING;
    ctx.fillText('🔥 INCINERATE: Use fire skills to destroy obstacles and rack up combos!', textX, lineY);
    lineY += LINE_SPACING;
    ctx.fillText('🎯 FLAWLESS: Finish with zero hits and all milestones for the ultimate victory!', textX, lineY);
    lineY += LINE_SPACING;
    ctx.fillText('📅 DAILY CHALLENGE: New challenge every day with unique themes & personas!', textX, lineY);
...
export function drawGameCounters() {
    ...
    // Use customGameIncinerationCount for custom persona games, consecutiveIncinerations for others
    const incinerationCount = (gameState.selectedPersona === 'custom' && !gameState.isDailyChallengeActive) 
        ? gameState.customGameIncinerationCount 
        : gameState.playerStats.consecutiveIncinerations;
    const hitsString = `Hits: ${gameState.hitsCounter} | 🔥: ${incinerationCount} (${gameState.currentSkillLevel} Skill)`;
    ...
    // Use the same incinerationCount for consistency (total for other games)
    const totalIncinerationCount = (gameState.selectedPersona === 'custom' && !gameState.isDailyChallengeActive) 
        ? gameState.customGameIncinerationCount 
        : gameState.playerStats.totalInGameIncinerations;
    const incineratedAndSkillText = ` | 🔥: ${totalIncinerationCount} (${gameState.currentSkillLevel} Skill)`;
